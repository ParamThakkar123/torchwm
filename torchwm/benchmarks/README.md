...
1. Run from the CLI (example):
   ...
   python -m torchwm.benchmarks.cli --agent iris --game ALE/Pong-v5 --seeds 1 --episodes 3
   
   # run DIAMOND on Breakout with two seeds
   python -m torchwm.benchmarks.cli --agent diamond --game Breakout-v5 --seeds 0,1 --episodes 5
   
   # run ALL agents on the same game for comparison
   python -m torchwm.benchmarks.cli --all-agents --game ALE/Pong-v5 --seeds 2 --episodes 5
   ```
...
- Add smoke tests under `torchwm/benchmarks/tests/` to validate adapters
