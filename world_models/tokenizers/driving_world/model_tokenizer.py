...
from world_models.tokenizers.driving_world.vq_model import VQ_models
...
class Tokenizer(nn.Module):
    def __init__(self, args, local_rank=-1):
        ...
        self.vq_model = VQ_models[args.vq_model](
            ...
            video_vq_temp_frames=args.video_vq_temp_frames,
        )
    ...
    def encode_to_z(self, x):
        b, t, _, _, _ = x.shape
        h, w = (
            self.args.image_size[0] // self.args.downsample_size,
            self.args.image_size[1] // self.args.downsample_size,
        )
...
class PoseTokenizer(nn.Module):
    def __init__(self, args, pose_codebook_size=4096):
        ...
        self.lati_range = [-1, 1]
        self.longtitu_seps = torch.arange(
            self.longti_range[0],
            self.longti_range[1],
            (self.longti_range[1] - self.longti_range[0]) / self.longtitute_bins,
        ).cuda()
        self.latitute_seps = torch.arange(
            self.lati_range[0],
            self.lati_range[1],
            (self.lati_range[1] - self.lati_range[0]) / self.latitute_bins,
        ).cuda()

    def tokenize_pose(self, poses):
        x, y = poses[:, :, 0], poses[:, :, 1]
        x_idx = (torch.searchsorted(self.longtitu_seps, x, side="right") - 1).clip(
            0, self.longtitute_bins - 1
        )
        y_idx = (torch.searchsorted(self.latitute_seps, y, side="right") - 1).clip(
            0, self.latitute_bins - 1
        )
    ...
    def poses_to_indices(self, poses):
        x, y = poses[:, :, 0], poses[:, :, 1]
        internal_x, internal_y = torch.floor(x * 16).clip(0, 127), torch.floor(
            (y + 1) * 16
        ).clip(0, 31)
