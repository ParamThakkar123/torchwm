...
from world_models.utils.drivingworld_utils import compute_axial_cis, apply_rotary_emb
from world_models.utils.drivingworld_utils import get_fourier_embeds_from_coordinates
...
class CausalTimeSpaceBlock(nn.Module):
    ...
    def forward(self, x, time_attn_mask, space_attn_mask):
        b, f, ll, c = x.shape
        x = rearrange(x, "b f ll c -> (b ll) f c")
        x = self.causal_time_block(x, time_attn_mask)
        x = rearrange(x, "(b ll) f c -> b f ll c", b=b, ll=ll, f=f)
        space_attn_mask = space_attn_mask.unsqueeze(1)
        space_attn_mask = space_attn_mask.repeat(1, x.shape[1], 1, 1)
        x = rearrange(x, "b f ll c -> (b f) ll c", b=b, f=f)
        space_attn_mask = rearrange(space_attn_mask, "b f ll c -> (b f) ll c")
...
class GPT(nn.Module):
    def __init__(
        ...
        self.causal_space_blocks = nn.Sequential(
            *[CausalSpaceBlock(config) for _ in range(self.auto_regressive_num)]
        )
        ...
        self.mask_ar_droppose[:, 1 : 1 + self.pose_token_size + self.yaw_token_size] = (
            float("-inf")
        )
    ...
    def get_yaw_pose_emb(self, pose_indices, yaw_indices):
        if pose_indices is None:
            ...
            yaw_emb, pose_x_emb = torch.split(
    ...
    def forward(
        ...
        query_embeddings = torch.cat(
            [
                sos_token_embeddings,
                add_yaw_embeddings,
                add_pose_x_embeddings,
                add_pose_y_embeddings,
                add_feature_embeddings[:, :, :-1, :],
            ],
            dim=2,
        )
        intra_query_embeddings = rearrange(
            query_embeddings, "B F L C -> (B F) L C", B=B, F=F
        )
    ...
    def eval_first_second(self, feature, pose_indices, yaw_indices, drop_flag):
        mask_spatial_curr, mask_ar_curr = self.organize_attn_mask(drop_flag)
        yaw_emb, pose_x_emb, pose_y_emb = self.get_yaw_pose_emb(
            pose_indices, yaw_indices
        )
        ...
        yaw_pose_scale_token_embeddings = torch.cat(
            [
                yaw_token_embeddings,
                pose_x_token_embeddings,
                pose_y_token_embeddings,
                feature_embeddings,
            ],
            dim=2,
        )
        ...
        auto_regressive_token_embeddings = rearrange(
            auto_regressive_token_embeddings, "B F L C -> (B F) L C", B=B, F=1
        )
    ...
    def eval_third(
        ...
        if (yaws is not None) and (poses is None) and (new_tokens is None):
            ...
            if poses.shape[2] == 2:
                ...
                intra_query_embeddings = torch.cat(
                    [
                        sos_embeddings,
                        yaw_embeddings,
                        pose_x_embeddings,
                ...
                intra_query_embeddings = torch.cat(
                    [sos_embeddings, yaw_embeddings, pose_x_embeddings], dim=1
                )
        ...
        for i in range(self.auto_regressive_num):
            auto_regressive_token_embeddings = self.causal_space_blocks[i](
                auto_regressive_token_embeddings, mask_ar_curr[:, :L1, :L1]
            )
        img_logits = None
        pose_x_logits = None
        pose_y_logits = None
        ...
        if L1 > 3:
            img_embed = auto_regressive_token_embeddings[
                :, (self.yaw_token_size + self.pose_token_size) :, :
            ]
            img_logits = self.head(img_embed)
        if L1 > 2:
            pose_y_embed = auto_regressive_token_embeddings[
                :,
                self.yaw_token_size + 1 : self.yaw_token_size + self.pose_token_size,
                :,
            ]
            pose_y_logits = self.pose_x_head(pose_y_embed)
        if L1 > 1:
            pose_x_embed = auto_regressive_token_embeddings[
                :, self.yaw_token_size : self.yaw_token_size + 1, :
            ]
