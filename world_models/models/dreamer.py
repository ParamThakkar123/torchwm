...
def make_env(args):
    ...
    if env_instance is not None:
    ...
    elif backend == "procgen":
        from world_models.envs import make_procgen_env

        env = make_procgen_env(
            args.env,
            distribution_mode=getattr(args, "procgen_distribution_mode", "hard"),
            num_levels=getattr(args, "procgen_num_levels", 0),
            start_level=getattr(args, "procgen_start_level", 0),
            seed=args.seed,
            size=size,
        )
        ...
        raise ValueError(
            f"Unknown env_backend='{backend}'. Use one of: dmc, gym, procgen, unity_mlagents."
        )
