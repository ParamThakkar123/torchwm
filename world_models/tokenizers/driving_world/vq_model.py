...
class VideoVQModel(nn.Module):
    ...
    def spatial_temporal_blocks(self, x):
        ...
        b, f, ll, c = xx.shape
        xx = rearrange(xx, "b f ll c -> (b ll) f c")
        xx = self.causal_time_block(xx, self.freqs_cis_time, mask_time)
        xx = rearrange(xx, "(b ll) f c -> (b f) ll c", b=b, ll=ll, f=f)
        xx = self.space_block(xx, self.freqs_cis_space)
        xx = rearrange(xx, "(b f) ll c -> b f ll c", b=b, f=f)
...
class VectorQuantizer(nn.Module):
    ...
    def forward_hacked(self, z):
        ...
        return (
            z_q,
            (vq_loss, commit_loss, entropy_loss, codebook_usage),
            (perplexity, min_encodings, min_encoding_indices, min_encoding_features),
        )
