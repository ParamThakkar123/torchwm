...
from world_models.utils.drivingworld_utils import (
    top_k_sampling,
    top_p_sampling,
    pk_sampling,
)
from world_models.tokenizers.driving_world.pose_tokenizer import (
    poses_to_indices,
    yaws_to_indices,
)
...
class TrainTransformers(nn.Module):
    def __init__(
        ...
        self.latent_size = (
            self.image_size[0] // self.args.downsample_size,
            self.image_size[1] // self.args.downsample_size,
        )
        ...
        self.total_token_size = (
            self.img_token_size + self.pose_token_size + self.yaw_token_size
        )
        ...
        transformer_config = {
            ...
            "n_layer": [args.n_layer, args.n_layer],
            "n_head": 16,
            "n_embd": args.n_embd,
        ...
        module_path = "world_models." + args.gpt_type
    ...
    def model_forward(
        ...
        logits = self.model(
            feature_total, pose_indices_total, yaw_indices_total, posedrop_input_flag
        )
        ...
        yaw_loss = nn.functional.cross_entropy(
            yaw_logits, targets[:, 0 : self.yaw_token_size], reduction="none"
        )
        pose_x_loss = nn.functional.cross_entropy(
            pose_x_logits,
            targets[:, self.yaw_token_size : self.yaw_token_size + 1],
            reduction="none",
        )
        pose_y_loss = nn.functional.cross_entropy(
            pose_y_logits,
            targets[:, self.yaw_token_size + 1 : self.yaw_token_size + 2],
            reduction="none",
        )
        ...
        img_loss = nn.functional.cross_entropy(
            img_logits,
            targets[:, self.yaw_token_size + self.pose_token_size :],
            reduction="none",
        )
        loss = torch.cat(
            [yaw_loss_weight, pose_x_loss_weight, pose_y_loss_weight, img_loss], dim=1
        )
        loss_mean = loss.mean()
        return loss_mean
    ...
    def step_train(
        ...
        pose_total = poses_to_indices(
            torch.cat([pose, pose_gt], dim=1),
            self.pose_x_vocab_size,
            self.pose_y_vocab_size,
        )
        yaw_total = yaws_to_indices(
            torch.cat([yaw, yaw_gt], dim=1), self.yaw_vocab_size
        )
        pro = random.random()
        if pro < self.args.mask_data:
            mask = torch.bernoulli(
                self.pkeep * torch.ones(token.shape, device=token.device)
            )
            mask = mask.round().to(dtype=torch.int64)
            random_token = torch.randint_like(token, self.codebook_size)
            new_token = mask * token + (1 - mask) * random_token
            mask_gt = torch.bernoulli(
                self.pkeep * torch.ones(token_gt.shape, device=token_gt.device)
            )
        ...
        feature_masked_total = torch.cat(
            [vqvae_codebook(new_token), vqvae_codebook(new_token_gt)], dim=1
        )
    ...
    def step_eval(
        ...
        intra_token_embeddings = self.model.eval_first_second(
            feature, yaw_indices, pose_indices
        )
        targets = torch.cat(
            [
                torch.cat([yaw_indices, pose_indices, token], dim=2),
        ...
        targets = targets[:, -1, ...]
        loss_final = 0
        logits_yaw = self.model.eval_third(
            intra_token_embeddings[:, : self.yaw_token_size, :]
        )["yaw_logits"]
        logits_yaw = logits_yaw[:, 0 : self.yaw_token_size, :]
        yaws = yaw_gt_indices[:, 0, :]
        loss_0 = nn.functional.cross_entropy(
            logits_yaw.reshape(-1, logits_yaw.size(-1)),
            targets[:, 0 : self.yaw_token_size].reshape(-1),
        )
        loss_final += loss_0
        logits_pose = self.model.eval_third(
            intra_token_embeddings[:, : self.yaw_token_size + self.pose_token_size, :],
            yaws=yaws,
        )["pose_logits"]
        logits_pose = logits_pose[
            :, self.yaw_token_size : self.yaw_token_size + self.pose_token_size, :
        ]
        poses = pose_gt_indices[:, 0, :]
        loss_1 = nn.functional.cross_entropy(
            logits_pose.reshape(-1, logits_pose.size(-1)),
            targets[
                :, self.yaw_token_size : self.yaw_token_size + self.pose_token_size
            ].reshape(-1),
        )
        ...
        for i in range(self.img_token_size):
            logits = self.model.eval_third(
                intra_token_embeddings[
                    :, : i + self.yaw_token_size + self.pose_token_size + 1, :
                ],
                yaws=yaws,
                poses=poses,
            ...
            logits = logits[:, i : i + 1, :]
            loss_tmp = nn.functional.cross_entropy(
                logits.reshape(-1, logits.size(-1)),
                targets[
                    :,
                    i
                    + self.yaw_token_size
                    + self.pose_token_size : i
                    + self.yaw_token_size
                    + self.pose_token_size
                    + 1,
                ].reshape(-1),
            )
            loss_final += loss_tmp
        ...
        predict_indices = rearrange(
            predict_indices,
            "b (h w) -> b h w",
            h=self.latent_size[0],
            w=self.latent_size[1],
        )
        indices_gt = rearrange(
            token_gt, "b 1 (h w) -> b h w", h=self.latent_size[0], w=self.latent_size[1]
        )
    ...
    def forward(
        ...
        if self.training:
            ...
            predict_indices, indices_gt, loss_final = self.step_eval(
                token,
                feature,
                pose,
                yaw,
    ...
    def generate_gt_pose_gt_yaw(
        ...
        pose_indices = poses_to_indices(
            pose, self.pose_x_vocab_size, self.pose_y_vocab_size
        )
        pose_gt_indices = poses_to_indices(
            pose_gt, self.pose_x_vocab_size, self.pose_y_vocab_size
        )
        yaw_indices = yaws_to_indices(yaw, self.yaw_vocab_size)
        yaw_gt_indices = yaws_to_indices(yaw_gt, self.yaw_vocab_size)
        intra_token_embeddings = self.model.eval_first_second(
            feature, pose_indices, yaw_indices, posedrop_input_flag
        )
        ...
        for i in range(self.img_token_size):
            ...
            if sampling_mtd == "top_k":
                idx_next = top_k_sampling(
                    logits, temperature_k=temperature_k, top_k=top_k
                )
            elif sampling_mtd == "top_p":
                idx_next = top_p_sampling(
                    logits, temperature_p=temperature_p, top_p=top_p
                )
            elif sampling_mtd == "pk":
                idx_next = pk_sampling(
                    logits,
    ...
    def save_model(self, path, epoch, rank=0):
        if rank == 0:
            torch.save(
                self.model.state_dict(), "{}/tvar_{}.pkl".format(path, str(epoch))
            )
