...
from world_models.datasets.nuplan.dataset_utils import get_meta_data
import torch
...
class NuPlanTest(Dataset):
    def __init__(
        ...
        self.meta_path = f"{json_root}/nuplan_test_meta.json"
        self.pose_meta_path = f"{json_root}/nuplan_test_pose_meta.json"
    ...
    def downsample_sequences(self, img_ts, poses):
        ...
        index_list = np.arange(0, ori_size, self.downsample)
    ...
    def getimg(self, index):
        ...
        for i in range(clip_length):
            im = cv2.cvtColor(
                cv2.imread(f"{rgb_front_dir}/{img_ts_downsample[i]}"), cv2.COLOR_BGR2RGB
            )
            h, w, _ = im.shape
            if 2 * h < w:
