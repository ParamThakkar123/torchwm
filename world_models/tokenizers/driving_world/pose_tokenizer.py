...
def poses_to_indices(poses, x_divisions=128, y_divisions=128):
    ...
    x, y = poses[:, :, 0], poses[:, :, 1]
    idx_x = (
        torch.floor(x * x_divisions / x_range)
        .clip(0, x_divisions - 1)
        .to(torch.long)
        .unsqueeze(dim=2)
    )
    idx_y = (
        torch.floor((y - y_min) * y_divisions / y_range)
        .clip(0, y_divisions - 1)
        .to(torch.long)
        .unsqueeze(dim=2)
    )
...
def yaws_to_indices(yaws, division=512):
    ...
    idx_yaw = (
        torch.floor((yaws - yaw_min) * division / yaw_range)
        .clip(0, division - 1)
        .to(torch.long)
    )
    return idx_yaw
